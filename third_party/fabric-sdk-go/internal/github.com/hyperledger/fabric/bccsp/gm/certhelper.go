...
import (
	...
	x5091 "github.com/tjfoc/gmsm/x509"
)
...
//调用SM2接口生成SM2证书
//func CreateCertificateToMem(template, parent *x5091.Certificate, key bccsp.Key) (cert []byte, err error) {
//	pk := key.(*gmsm2PrivateKey).privKey
//
...
//		puk.X = pub.X
//		puk.Y = pub.Y
//		cert, err = x5091.CreateCertificateToPem(template, parent, &puk, pk)
//
//	}
//	return
//}
...
//调用SM2接口生成SM2证书请求
func CreateSm2CertificateRequestToMem(certificateRequest *x5091.CertificateRequest, key bccsp.Key) (csr []byte, err error) {
	pk := key.(*gmsm2PrivateKey).privKey
	csr, err = x5091.CreateCertificateRequestToPem(certificateRequest, pk)
	return
}

// X509 证书请求转换 SM2证书请求
func ParseX509CertificateRequest2Sm2(x509req *x509.CertificateRequest) *x5091.CertificateRequest {
	sm2req := &x5091.CertificateRequest{
		...
		SignatureAlgorithm: x5091.SignatureAlgorithm(x509req.SignatureAlgorithm),

		PublicKeyAlgorithm: x5091.PublicKeyAlgorithm(x509req.PublicKeyAlgorithm),
...
// X509证书格式转换为 SM2证书格式
func ParseX509Certificate2Sm2(x509Cert *x509.Certificate) *x5091.Certificate {
	sm2cert := &x5091.Certificate{
		Raw:                     x509Cert.Raw,
		...
		SignatureAlgorithm: x5091.SignatureAlgorithm(x509Cert.SignatureAlgorithm),

		PublicKeyAlgorithm: x5091.PublicKeyAlgorithm(x509Cert.PublicKeyAlgorithm),
		...
		KeyUsage:     x5091.KeyUsage(x509Cert.KeyUsage),
	...
	for _, val := range x509Cert.ExtKeyUsage {
		sm2cert.ExtKeyUsage = append(sm2cert.ExtKeyUsage, x5091.ExtKeyUsage(val))
	}
...
//sm2 证书转换 x509 证书
func ParseSm2Certificate2X509(sm2Cert *x5091.Certificate) *x509.Certificate {
	if sm2Cert == nil {
		return nil
	}
