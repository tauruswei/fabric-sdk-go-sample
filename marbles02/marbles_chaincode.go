...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "lock" {
		return t.lock(stub, args)
	} else if function == "unlock" {
		return t.unlock(stub, args)
	}
	// 判断chaincode有没有被锁
	state, err := stub.GetState("lock")
	if err != nil {
		return shim.Error(err.Error())
	}

	if strings.EqualFold("false", string(state)) || string(state) == "" {
		...
		} else if function == "changeSize" {
			return t.changeSize(stub, args)
		}
		fmt.Println("invoke did not find func: " + function) //error
		return shim.Error("Received unknown function invocation")
	} else {
		return shim.Error("marble chaincode is locked")
	}
}

// ============================================================
// lock - lock marbles chaincode
// ============================================================
func (t *SimpleChaincode) lock(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	...
	if err != nil {
		...
		if err != nil {
			return shim.Error(fmt.Sprintf("parse certificate error :%s", certificate))
		}
	...
	fmt.Println(fmt.Sprintf("subject = %s", certificate.Subject.String()))

	if strings.Contains(strings.ToLower(commonName), "admin") {
		err := stub.PutState("lock", []byte("true"))
		if err != nil {
			return shim.Error(err.Error())
		}
	} else {
		return shim.Error("no permission to perform lock")
	}
	fmt.Println("- end lock marble")
	return shim.Success(nil)
}

// ============================================================
// unlock - unlock marbles chaincode
// ============================================================
func (t *SimpleChaincode) unlock(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	...
	if strings.Contains(strings.ToLower(commonName), "admin") {
		err := stub.PutState("lock", []byte("false"))
		if err != nil {
			return shim.Error(err.Error())
		}
	} else {
		return shim.Error("no permission to perform unlock")
	}
	fmt.Println("- end unlock marble")
	return shim.Success(nil)
}
...
func (t *SimpleChaincode) initMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	stub.SetEvent("test", marbleJSONasBytes)
...
func (t *SimpleChaincode) changeSize(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	fmt.Println("======== begin change size =========")
	var name, jsonResp string
	var err error
	var marbleJSON marble
	...
	fmt.Println(fmt.Sprintf("marble = %v", marbleJSON))

	newsize, err := strconv.Atoi(args[1])

	marbleJSON.Size = marbleJSON.Size - newsize

	marbleJSONasBytes, _ := json.Marshal(marbleJSON)
	err = stub.PutState(marbleJSON.Name, marbleJSONasBytes) //rewrite the marble

	if err != nil {
		return shim.Error(err.Error())
	}

	fmt.Println("======== end change size =========")
	return shim.Success(marbleJSONasBytes)
}
